...
codiguera_ncm <- function(){
  ...
  ncm_base %>%
    ...
    dplyr::filter(variable == 'ncm_10') %>%
    dplyr::transmute(ncm_10 = codigo, desc.ncm_10 = descripcion) %>%
    dplyr::mutate(ncm_9 = str_sub(ncm_10, 1, 9)) %>%

    left_join(ncm_base %>%
                dplyr::filter(variable == 'ncm_9') %>%
                dplyr::transmute(ncm_9 = codigo, desc.ncm_9 = descripcion),
              by = 'ncm_9') %>%
    dplyr::mutate(ncm_8 = str_sub(ncm_9, 1, 8)) %>%
    ...
    dplyr::left_join(ncm_base %>%
                       dplyr::filter(variable == 'seccion') %>%
                       dplyr::transmute(seccion = codigo, desc.secc = descripcion),
                     by = 'seccion') %>%
...
codiguera_ncm_wide <- function(){
  ...
  ncm_base %>%
    ...
    dplyr::filter(variable == 'ncm_10') %>%                                                               # Empieza por mayor desagregación ncm a 10 dígitos
    dplyr::transmute(ncm_10 = codigo, desc.ncm_10 = descripcion) %>%
    dplyr::mutate(ncm_9 = str_sub(ncm_10, 1, 9)) %>%

    dplyr::left_join(
      ncm_base %>%
        dplyr::filter(variable == 'ncm_9') %>%                                                            # Agrega ncm a 9 dígitos
        dplyr::transmute(ncm_9 = codigo, desc.ncm_9 = descripcion),
      by = 'ncm_9') %>%
    dplyr::mutate(ncm_8 = str_sub(ncm_9, 1, 8)) %>%

    dplyr::left_join(ncm_base %>%
                dplyr::filter(variable == 'ncm_8') %>%                                                    # Agrega ncm a 8 dígitos
                dplyr::transmute(ncm_8 = codigo, desc.ncm_8 = descripcion),
              by = 'ncm_8') %>%
    dplyr::mutate(ncm_7 = str_sub(ncm_8, 1, 7)) %>%
    ...
    asigna.seccion() %>%
    dplyr::mutate(seccion = as.character(seccion)) %>%                                             # Agrega sección según df "secciones.productos"
    dplyr::left_join(secciones.productos %>%
                       dplyr::mutate(seccion = as.character(seccion)) %>%
                       dplyr::select(desc.secc = descripcion.seccion, everything()),
                     by = "seccion") %>%
    ...
    dplyr::mutate(desc.cap    = str_replace_all(desc.cap,    pattern = "- ", ""),                         # elimina guiones de las descripciones de las partidas de 4, 5, 6, 7, 8, 9 y 10 dígitos
    ...
    dplyr::mutate(descripcion_completa = str_c(ifelse(is.na(desc.secc) == TRUE, "", desc.secc),
    ...
    dplyr::mutate(desc.ncm_9 = case_when(is.na(desc.ncm_9) == TRUE ~ desc.ncm_10, TRUE ~ desc.ncm_9),      # Completa descripciones faltantes
...
codiguera_ncm_long <- function(){
  codiguera_ncm_wide() %>%
    ...
    rbind(codiguera_ncm_wide() %>%
            ...
            dplyr::transmute(variable = "ncm_8", codigo = ncm_8, descripcion = desc.ncm_8, by = ncm_9) %>%
            unique) %>%

    rbind(codiguera_ncm_wide() %>%
            dplyr::transmute(variable = "ncm_9", codigo = ncm_9, descripcion = desc.ncm_9, by = ncm_10) %>%
            unique) %>%

    rbind(codiguera_ncm_wide() %>%
            dplyr::transmute(variable = "ncm_10", codigo = ncm_10, descripcion = desc.ncm_10, by = NA) %>%
            unique)
}
